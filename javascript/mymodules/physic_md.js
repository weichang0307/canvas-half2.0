...
class physic_world{
	...
	delete_constraint(a,b,origin_dis,count){
		...
		for(let i in this.constraint){
			if(JSON.stringify(ex)===JSON.stringify(this.constraint[i])){
	...
	update(time){
		...
		for(let k=0;k<this.iteration;k++){
			...
			for(let i of this.constraint){
				...
				i.a.velocity.add_in(new vec2(0,0).set(ff,dis.deg()+Math.PI).scale(time_/i.a.mass))
				i.b.velocity.add_in(new vec2(0,0).set(ff,dis.deg()).scale(time_/i.b.mass))
